  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled
          ? "bg-black/90 backdrop-blur-md shadow-lg shadow-yellow-500/10"
          : "bg-transparent"
      }`}
    >
          ...
          {user ? (
            <>
              <div className="flex items-center space-x-2 sm:space-x-3">
                ...
                {/* Payment Pending Button - Desktop */}
                {isPaymentPending && (
                  <button
                ...
                {/* Active Ride Button - Mobile (visible in navbar) */}
                {rideData && !isPaymentPending && (
                  <button
                    onClick={handleRideMapNavigation}
                    className={`flex sm:hidden items-center space-x-1 px-2 py-1.5 rounded-full font-medium transition-all duration-300 ${
                      isPulsing
                        ? "bg-green-500 shadow-lg shadow-green-500/50"
                        : "bg-green-600 shadow-md"
                    }`}
                ...
                {/* Payment Pending Button - Mobile (visible in navbar) */}
                {isPaymentPending && (
                  <button
                    onClick={handlePaymentNavigation}
                    className="flex sm:hidden items-center space-x-1 px-2 py-1.5 rounded-full bg-amber-500 font-medium transition-all duration-300 shadow-lg shadow-amber-500/30 animate-pulse"
                  >
                ...
                <div className="relative">
                  ...
                  >
                    <Bell className="h-5 w-5 text-[#fdb726] group-hover:text-[#fdb726] transition-colors" />
                    {unreadCount > 0 && (
